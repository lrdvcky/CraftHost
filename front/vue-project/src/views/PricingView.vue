    ...
    <template v-else>
      ...
      <div class="plans-grid">
        ...
        >
          ...
          <router-link :to="{ path: '/order', query: { tariff: t.id } }" class="soft-button w-full" :class="isFeatured(i) ? 'primary' : 'secondary'">
            Выбрать тариф
          </router-link>
        </div>
      </div>
...
const metaMap = {
  ...
  Stone: {
    ...
    desc: 'Базовый тариф для маленьких ванильных серверов и тестов.',
  ...
  Netherite: {
    icon: 'Nether_Star_JE2_BE1.png',
    tagline: 'Топ-конфигурация',
    desc: 'Максимальная мощность для крупных публичных серверов с сотнями игроков.',
    features: ['Любые модпаки и плагины', 'До 8 GB RAM, 300% vCPU', 'Приоритетная поддержка 24/7'],
  },
  Emerald: {
    icon: 'Emerald_JE3_BE3.png',
    tagline: 'Для большого онлайна',
    desc: 'Премиум-тариф для проектов с большой нагрузкой и постоянным онлайном.',
    features: ['Большой стабильный онлайн', 'Расширенные ресурсы', 'Приоритетная поддержка'],
  },
  Crazy: {
    icon: 'Nether_Star_JE2_BE1.png',
    tagline: 'Безумный сервер',
    desc: 'Топ железо без компромиссов.',
    features: ['Максимум RAM и vCPU', 'NVMe-диск', 'Поддержка 24/7'],
  },
}
const fallbackMeta = {
  icon: 'Grass_Block_JE4.png',
...
@media (max-width: 768px) {
  .pricing-page { padding: 30px 0 20px; }
  .pricing-head { margin-bottom: 28px; }
  .plans-grid { grid-template-columns: 1fr; gap: 16px; margin-bottom: 50px; }
  .plan-card { padding: 24px 22px; }
  .plan-card:hover { transform: none; }
  .pp-value { font-size: 32px; }
  .included-section, .compare-section { margin-bottom: 50px; }
...
@media (max-width: 600px) {
  ...
  .cta-actions .soft-button { width: 100%; }
}
</style>
