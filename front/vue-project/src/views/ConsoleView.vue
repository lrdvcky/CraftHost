...
// Локальные строки, добавленные пользователем (эхо команд / служебные).
// Перетекают каждый поллинг — если в серверном логе появилось то же — заменяются.
...
const fetchConsole = async () => {
  ...
  try {
    ...
    if (r.data.logs?.length) {
      // Замещаем строки на актуальные с сервера + добавляем локальные
      // (которые ещё не отразились в server.log).
      const wasAtBottom = termBox.value
        ? (termBox.value.scrollTop + termBox.value.clientHeight + 30 >= termBox.value.scrollHeight)
        : true

      // Сбрасываем локальные «эхо», если последняя строка с сервера новее
      // (по содержимому совпадает с одним из локальных эхо).
      localLines.value = localLines.value.filter(local => {
        // Если за последние 8 строк сервер уже показал отклик — убираем эхо
        const recent = r.data.logs.slice(-8).map(l => l.text)
        return !recent.some(t => t.includes(local.text.replace(/^>\s*/, '').slice(0, 40)))
      })

      logLines.value = [...r.data.logs, ...localLines.value]
      if (wasAtBottom) await scrollDown()
    }
  } catch {}
}
...
const sendCommand = async () => {
  ...
  const echo = { time: timeStr, level: 'CMD', text: `> ${cmd}` }
  localLines.value.push(echo)
  logLines.value.push(echo)
  await scrollDown()

  try {
    await api.post(`/servers/${serverId}/command`, { command: cmd })
    // Через секунду — внеочередной поллинг, чтобы быстрее показать ответ
    setTimeout(fetchConsole, 1200)
    setTimeout(fetchConsole, 3000)
  } catch (e) {
    const errLine = { time: timeStr, level: 'ERROR', text: e.response?.data?.error || 'Ошибка отправки' }
    localLines.value.push(errLine)
    logLines.value.push(errLine)
  } finally {
...
onMounted(async () => {
  ...
  if (!error.value) {
    await fetchConsole()
    pollHandle = setInterval(fetchConsole, 3000)
  }
})
...
.tl-level.cmd { color: var(--mc-emerald); }
.term-line.cmd .tl-text { color: var(--mc-emerald); font-weight: 600; }
...
@media (max-width: 768px) {
  .console-page { padding: 24px 0; }
  .console-actions { flex-wrap: wrap; gap: 8px; }
  .console-actions .soft-button { flex: 1; min-width: 100px; }
